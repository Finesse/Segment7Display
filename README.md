...
Выводит на дисплей строку текста. Будут отображаться только символы, указанные в описании к методу printChar, остальные будут заменены на пробел. После выполнения этого метода режим декодирования будет выключен.

Параметры:
* `String str` Строка, которую нужно вывести
* `byte align` Сторона, к которой примыкает строка. Возможные варианты располагаются в статичных константах `ALIGN_*` этого класса. По-умолчанию, строка примыкает к правой стороне.
...
* `int align` Сторона, к которой примыкает строка. Возможные варианты располагаются в статичных константах `ALIGN_*` этого класса. По-умолчанию, строка примыкает к правой стороне.
* `boolean clearExcess` Нужно ли очистить блоки цифр, не участвующие в написании этой строки. По-умолчанию, очищаются.
* `int offset` Сдвиг строки относительно прилегаемого края, к которому она прилегает. По-умолчанию, сдвига нет.
* `int maxLength` Максимальная длина строки. Если указать значение меньше нуля, то ограничения по длине не будет. По-умолчанию, ограничения нет.
...
Используются для передачи в методы `printString` и `printFloat`.

#### ALIGN_LEFT
Строка будет примыкать к левой стороне. Лишние символы будут обрезаны справа.

#### ALIGN_RIGHT
Строка будет примыкать к правой стороне. Лишние символы будут обрезаны слева.